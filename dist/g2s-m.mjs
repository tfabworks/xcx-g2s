...
var en = {
	...
	"g2s.connectSpreadsheetAppend": "spreadsheet(β) append [VALUE] to URL [URL]",
	"g2s.connectSpreadsheetWrite": "spreadsheet(β) write [VALUE] to URL [URL] at column [COLUMN], row [ROW]",
	"g2s.connectSpreadsheetRead": "spreadsheet(β) value at column [COLUMN], row [ROW] in URL [URL]",
	"g2s.connectSendLine": "LINE(β) send [MESSAGE] to token [DESTINATION]",
	"g2s.connectSendMail": "email(β) send [MESSAGE] to address [DESTINATION]",
...
var ja = {
	...
	"g2s.connectSpreadsheetAppend": "スプレッドシート(β) URL[URL]に[VALUE]を記録",
	"g2s.connectSpreadsheetWrite": "スプレッドシート(β) URL[URL]の[COLUMN]列[ROW]行に[VALUE]を書き込む",
	"g2s.connectSpreadsheetRead": "スプレッドシート(β) URL[URL]の[COLUMN]列[ROW]行の値",
	"g2s.connectSendLine": "LINE(β) トークン[DESTINATION]に[MESSAGE]を送る",
	"g2s.connectSendMail": "メール(β) アドレス[DESTINATION]に[MESSAGE]を送る",
...
var translations = {
	...
	"g2s.connectSpreadsheetAppend": "スプレッドシート(β) URL[URL]に[VALUE]をきろく",
	"g2s.connectSpreadsheetWrite": "スプレッドシート(β) URL[URL]の[COLUMN]れつ[ROW]ぎょうに[VALUE]をかきこむ",
	"g2s.connectSpreadsheetRead": "スプレッドシート(β) URL[URL]の[COLUMN]れつ[ROW]ぎょうのあたい",
	"g2s.connectSendLine": "LINE(β) トークン[DESTINATION]に[MESSAGE]をおくる",
          ...
          while (1) {
            switch (_context21.prev = _context21.next) {
              ...
              case 5:
                ...
                setTimeout(function () {
                  _this29.cachedBoardInfo.expired = true;
                }, 10 * 60 * 1000);
          ...
          while (1) {
            switch (_context22.prev = _context22.next) {
              ...
              case 3:
                ...
                if (!document.cookie.includes("".concat(this.akaDakoConnectLimitCookieName, "="))) {
                  token = _toConsumableArray(Array(20)).map(function () {
                    return "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random() * 62));
                  }).join("");
                  document.cookie = "".concat(this.akaDakoConnectLimitCookieName, "=").concat(token, "; Path=/; max-age=63072000; Secure");
                }
                ...
                return fetch(url, {
                  ...
                  return typeof body["Ok"] !== 'undefined' && body["Ok"] !== null ? body["Ok"] : typeof body["Err"] !== 'undefined' && body["Err"] !== null ? body["Err"] : '';
                }).catch(function (e) {
                  var msg = formatMessage({
