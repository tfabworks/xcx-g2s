...
const guiPath = process.env.XCRATCH_PATH || '../scratch-gui';

// Build a normal module
execSync(`npx xcratch-build --module=g2s --gui=${guiPath} --block=./src/vm/extensions/block --entry=./src/gui/lib/libraries/extensions/entry --output=./build`);
...
try {
    execSync(`npx xcratch-build --module=g2s-m --gui=${guiPath} --block=./src/vm/extensions/block --entry=./src/gui/lib/libraries/extensions/entry --output=./build`);
